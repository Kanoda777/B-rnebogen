...
import { Library as LibraryIcon, BookOpen, Plus, UploadCloud, Edit, Trash2 } from "lucide-react";
...
export default function Library() {
    ...
    const handleDeleteStory = async (storyId) => {
        if (window.confirm("Er du sikker på, at du vil slette denne historie?")) {
            try {
                await Story.delete(storyId);
                loadStories(); // Reload the list
            } catch (error) {
                console.error("Failed to delete story:", error);
                alert("Kunne ikke slette historie. Prøv igen.");
            }
        }
    };
    ...
    const handlePublish = async (story, consentData) => {
        try {
            await SharedStory.create({
                ...
                author_age: consentData.authorAge,
                title: story.title,
            ...
            alert("Der skete en fejl ved udgivelsen. Prøv igen.");
        }
    };
    ...
    return (
        <SafePageWrapper requiresProfile={false} pageName="Library">
                                    ...
                                    <div className="p-6 flex flex-col flex-grow">
                                        ...
                                        <div className="flex flex-col gap-2 mt-auto">
                                            ...
                                            <div className="flex gap-2">
                                                <Link to={createPageUrl(`EditStory?id=${story.id}`)} className="flex-1">
                                                    <Button 
                                                        variant="outline"
