...
export default function CreateChildForm({ onChildCreated, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Defensive programming: Ensure newChild and its ID exist before using
      if (newChild && newChild.id) {
        selectActiveChild(newChild.id); // Set the new child as active
        onChildCreated();
      } else {
        console.error("Error: Child.create returned an invalid object or a null ID.", newChild);
        alert("Der opstod en fejl ved oprettelse af barneprofilen. Prøv venligst igen.");
      }
    } catch (error) {
      console.error("Error creating child:", error);
      alert("Der opstod en fejl ved oprettelse af barneprofilen: " + error.message);
    } finally {
