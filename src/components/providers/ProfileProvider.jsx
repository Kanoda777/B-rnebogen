...
export const ProfileProvider = ({ children }) => {
    ...
    // Only fetch child data when we have a valid ID AND have checked storage
    useEffect(() => {
        const fetchChildData = async () => {
            // Triple check we have a valid ID before making any API calls
            if (!hasCheckedStorage || !activeChildId || activeChildId === 'null' || activeChildId === 'undefined' || activeChildId.trim() === '') {
                setActiveChild(null);
                setIsLoading(false);
                return;
            }
            ...
            try {
                ...
                const foundChild = childrenList.find(child => child && child.id === activeChildId);
        ...
        fetchChildData();
    }, [activeChildId, hasCheckedStorage]);

    const selectActiveChild = (childId) => {
        // Validate childId before setting
        if (!childId || childId === 'null' || childId === 'undefined' || typeof childId !== 'string' || childId.trim() === '') {
